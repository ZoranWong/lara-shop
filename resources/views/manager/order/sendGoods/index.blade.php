    ...
    <div class="box">
      <div>
          <table class="table table-bordered" style="text-align: center;">
              ...
              <tbody id="content-sendGoods"></tbody>
          </table>
        ...
        function initFunctionSendGoods(paramsJSON){
            $.get('/ajax/orders/index', paramsJSON, function(json){
                ...
                for(var i=0; i<data.length; i++) {
                    ...
                    for(var j=0; j<data[i].orders_item.length; j++){
                        ...
                        switch(data[i].orders_item[j].refundStatus)
                        {
                          ...
                          case 60 :
                            refundCase = '退款成功';
                            refundMoneyCase = "<a style='border:1px red solid;font-size:5px;color:red;width:60px;height:15px;' href='#'>钱款去向</a>";
                            break;
                        }
                ...
                function getPaginate(nowPage,pageCount){
                    ...
                    //首页显示
                    pageHtml += '<button value="1" class="pageList_sendGoods">首页</button>';
                    //上一页判断
                    if( nowPage > 1 ){

                        pageHtml += '<button class="pageList_sendGoods" value="' + (nowPage-1) + '">上一页</button>';
                        if((nowPage-2)>0){
                            pageHtml += '<button class="pageList_sendGoods" value="' + (nowPage-2) + '">' + (nowPage-2) + '</button>';
                        }

                        pageHtml += '<button class="pageList_sendGoods" value="' + (nowPage-1) + '">' + (nowPage-1) + '</button>';
                    }
                    //当前页显示
                    pageHtml += '<button value="' + nowPage + '"><span style="font-size:16px; color:red;">' + nowPage + '</span></button>';
                    //下一页判断
                    if( nowPage < pageCount ){
                        pageHtml += '<button class="pageList_sendGoods" value="' + (parseInt(nowPage)+1) + '">' + (parseInt(nowPage)+1) + '</button>';
                        if((parseInt(nowPage)+2) <= pageCount){
                            pageHtml += '<button class="pageList_sendGoods" value="' + (parseInt(nowPage)+2) + '">' + (parseInt(nowPage)+2) + '</button>';
                        }
                        pageHtml += '<button class="pageList_sendGoods" value="' + (parseInt(nowPage)+1) + '">下一页</button>';
                    }
                    pageHtml += '<button class="pageList_sendGoods" value="' + pageCount + '">末页</button>';
                    pageHtml += "</td></tr>";
                    return pageHtml;

                }
                $('.pageList_sendGoods').click(function(){
                    ...
                    initFunctionSendGoods(paramPage);
                });
            });
        }
        initFunctionSendGoods(param);
