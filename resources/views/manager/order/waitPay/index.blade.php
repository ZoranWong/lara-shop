    ...
    <div class="box">
      <div>
          <table class="table table-bordered" style="text-align: center;">
              ...
              <tbody id="content-waitPay"></tbody>
          </table>
      </div>
    <!-- /.box-body -->
  </div>
        ...
        function initFunctionWaitPay(paramsJSON){
            $.get('/ajax/orders/index', paramsJSON, function(json){
                ...
                function getPaginate(nowPage,pageCount){
                    ...
                    //首页显示
                    pageHtml += '<button value="1" class="pageList_waitPay">首页</button>';
                    //上一页判断
                    if( nowPage > 1 ){

                        pageHtml += '<button class="pageList_waitPay" value="' + (nowPage-1) + '">上一页</button>';
                        if((nowPage-2)>0){
                            pageHtml += '<button class="pageList_waitPay" value="' + (nowPage-2) + '">' + (nowPage-2) + '</button>';
                        }

                        pageHtml += '<button class="pageList_waitPay" value="' + (nowPage-1) + '">' + (nowPage-1) + '</button>';
                    }
                    //当前页显示
                    pageHtml += '<button value="' + nowPage + '"><span style="font-size:16px; color:red;">' + nowPage + '</span></button>';
                    //下一页判断
                    if( nowPage < pageCount ){
                        pageHtml += '<button class="pageList_waitPay" value="' + (parseInt(nowPage)+1) + '">' + (parseInt(nowPage)+1) + '</button>';
                        if((parseInt(nowPage)+2) <= pageCount){
                            pageHtml += '<button class="pageList_waitPay" value="' + (parseInt(nowPage)+2) + '">' + (parseInt(nowPage)+2) + '</button>';
                        }
                        pageHtml += '<button class="pageList_waitPay" value="' + (parseInt(nowPage)+1) + '">下一页</button>';
                    }
                    pageHtml += '<button class="pageList_waitPay" value="' + pageCount + '">末页</button>';
                    pageHtml += "</td></tr>";
                    return pageHtml;

                }
                $('.pageList_waitPay').click(function(){
                    ...
                    initFunctionWaitPay(paramPage);
                });
            });
        }
        initFunctionWaitPay(param);
