...
<div class="row" style="margin-bottom: 20px;">
    ...
    <div class="col-xs-6">
        <div style="height:50px; line-height: 15px;">
...
@push('js')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            var id =  {{ $id }};
            //填充表单数据
            $.getJSON('/ajax/shop/order/detail/'+id,function (res) {
                var data = new Object();
                ...
                for(var i = 0; i< data.total; i++) {
                    ...
                    tableRows += '<td colspan="2" style="text-align:left;"><img src="' + data.rows[i].pic_thumb_path + '" style="width:80px;height:80px;border-radius: 15%;margin-left:50px;" onerror=""><span style="width:200px; height:60px; line-height:30px; position:absolute; margin-top:10px; text-align:center; font-size:13px;">' + data.rows[i].name + '<br>' + data.rows[i].sku_properties_name +'</span>  <span style="position: absolute;margin-left: 200px;margin-top: 10px;text-align: center;width: 80px;height: 60px;padding-top: 17px;">' + data.rows[i].priceAmount +'</span></td>';
                    ...
                    tableRows += '<td style="text-align:center;padding-top:35px;" class="signed_at">' + html + '</td>';
                    tableRows += '</tr>';
                }
                ...
                switch(res.data.status)
                {
                    ...
                    case 40: var html = '已完成';
                             var tip = '交易完成，如果买家提出售后要求，请积极自行于买家协商，做好售后服务。';
                              var post_company = res.data.post_name + '&nbsp;&nbsp;&nbsp;运单号: &nbsp;&nbsp;&nbsp;' + res.data.post_no;
            ...
            $('#close').click(function(){
                if(confirm('确定关闭该订单吗？')){
                    $.post('/ajax/shop/order/cancel', {'id': id}, function(data){
                        if(data.code == 200) {
                            $.notify({
                                message: '关闭订单成功'
                            },{
