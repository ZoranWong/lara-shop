...
$(document).ready(function () {
  ...
  $("#category-tbody").bind('click','.btn',function(e){
    ...
    if(e.target.getAttribute('name') == 'subEdit'){
      ...
      $.post('/ajax/shop/category/save/'+id, {name:category}, function (json) {
        var data = json.data;
        if(json.code == 200){
          ref();
    ...
    if(e.target.getAttribute('name') == 'delete'){
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
          if(result){
            $.post('/ajax/shop/category/delete', {id:[id]}, function (json) {
              var data = json.data;
              if(json.code == 200){
  ...
  $("#category-tbody").bind('click','#sub-category',function(e){
    ...
    $.post('/ajax/shop/category/create', {name:category,parent_id:0}, function (json) {
      var data = json.data;
      if(json.code == 200){
        ref();
      }else if (json.code == 401) {
