...
<div class="box-body">
    <table id="audit_table"
           ...
           data-url="/ajax/shop/fenxiao/cash/list?status=0"
...
<script type="text/template" id="audit_tpl">
    ...
    <div class="audit_detail_info"><span style="float:left;">提现详情</span>
        ...
        <div class="audit_post_money" >打款</div>
    </div>
    <div class="tree well">
    ...
    <script>
        $(function() {
            $('.audit_post_money').click(function() {
                var audit_fans_id = $(".audit_user_id").text();
                var audit_pay_cash_id = $("#audit_pay_id").text();
                var audit_pay_mobile = $("#audit_pay_tel").text();
                var productList = $("#audit_pay_tpl").text();
                bootbox.alert({
                    title: '<span class="text-success">打款进度</span>',
                    message: productList
                });
                $("#audit_pay_fans_id").append(audit_fans_id);
                $("#audit_pay_cash_id").append(audit_pay_cash_id);
                var audit_pay_cash_id = $("#audit_pay_cash_id").text();
                var audit_pay_fans_id = $("#audit_pay_fans_id").text();
                /*申请提现金额 等待打款金额*/
                var audit_amount_apply = $("#audit_amount_apply").text();
                var audit_wait = $("#audit_wait").text();
                $('.audit_amount_extract').append(audit_amount_apply);
                $('.audit_amount_wait').append(audit_wait);
                $('#autit_alipay_account').val(audit_pay_mobile);
            });
        });
        $(".modal-footer").remove();
    </script>
...
<script type="text/template" id="audit_pay_tpl">
    ...
    <div class="audit_operation">
        ...
        <div class="audit_pay_true">确认</div>
    </div>
    <style>
    ...
    $(document).ready(function () {
        ...
        window.operateEvents = {
            ...
            'click .detail': function()
            {
                ...
                $.post('/ajax/shop/fenxiao/cash/detail?status=0', {'cash_id':cash_id,'fans_id':fans}, function (json)
                {
                    if(json.code=200)
                    {
                        var data=json.data;
