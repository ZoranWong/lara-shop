...
class OrderController
{
    ...
    public function detail($id)
    {
        try{
            $storeId = StoreService::getCurrentID();
            $data = Order::with(['weappUser' => function($query){
                $query->select(['id','nickname']);
            },'ordersItem.refundApply'=>function($query){$query->select('status','money','order_item_id')->orderBy('id','desc');},'userCoupon'])->where('store_id',$storeId)->where('id',$id)->first();
            if(!$data){
                throw new Exception('订单不存在', 404);
            }
            $data->nickname = '-';
            if($data->weappUser){
                $data->nickname = $data->weappUser->nickname; //买家昵称
                unset($data->weappUser);
            }
            $data->refunded_fee = 0;
            if($data->ordersItem){
            ...
            $data->signed_at = !empty($data->signed_at) ? date('Y-m-d H:i:s',$data->signed_at) : "-";
            $data->paid_at = !empty($data->paid_at) ? date('Y-m-d H:i:s',$data->paid_at) : "-";
            $data->consigned_at = !empty($data->consigned_at) ? date('Y-m-d H:i:s',$data->consigned_at) : "-";
