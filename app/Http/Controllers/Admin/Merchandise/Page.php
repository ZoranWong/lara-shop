...
use App\Http\Controllers\Admin\Merchandise\Actions\OnShelvesAction;
use App\Http\Controllers\Admin\Merchandise\Tools\OnShelvesBatch;
use App\Http\Controllers\Admin\Merchandise\Tools\TakenOffBatch;
...
class Page extends BasePage
{
    ...
    protected function filter(Grid\Filter $filter)
    {
        ...
        $filter->between('sell_price', '商品价格')->currency([
            'min' => 0.00,
            'step' => 0.01
        ]);
    ...
    public function vueForm($id = null)
    {
        SectionContent::link(asset('/vue/css/app.css'));
        SectionContent::jsLoad(asset('/vue/js/app.js'));
        $merchandise = ($id ? Merchandise::with(['products'])->find($id) : null);
        if ($merchandise){
            $products = Merchandise::findForStore($merchandise['store_id']);
        }
        $this->box->content(new MerchandiseEditor($merchandise));
        return $this;
    }
...
CSS;
        ...
        $grid->tools(function (Grid\Tools $tools){
            ...
            $tools->batch(function (Grid\Tools\BatchActions $actions){
                $actions->add('批量下架', new TakenOffBatch());
            });
        });
        $grid->id('ID')->sortable();
        $grid->code('商品编号');
...
HTML;
        ...
        $grid->market_price('原价')->sortable();
        $grid->sell_price('单价')->sortable();
