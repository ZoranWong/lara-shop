...
class CommissionController extends Controller
{
    ...
    public function distributionCommissionLevelEdit(Request $request)
    {
        ...
        if( $id ){
            $data = CommissionLevel::where('store_id', $storeId)->where('id', $id)->get(['level', 'commission_status']);
            return \Response::ajax($data);
        } else {
            return \Response::ajax(['level' => CommissionLevel::where('store_id', $storeId)->max('level') + 1, 'commission_status' => 0]);
        }
    }
    ...
    public function saveCommissionLevel(Request $request)
    {
        $input = $this->validate($request, [
            'id'  => ['integer'],
            'name' => ['required', 'string', Rule::unique('commission_level', 'name')],
            'level' => ['required', 'integer'],
            'upgrade_type' => ['required', 'integer'],
            'reach_amount' => 'numeric|min:0',
            'allocation'  => [ 'integer', Rule::in(CommissionLevel::ALLOCATION)],
            'commission_status' => ['integer', Rule::in(CommissionLevel::COMMISSION_STATUS)],
