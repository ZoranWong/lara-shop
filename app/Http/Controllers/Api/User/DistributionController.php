...
class DistributionController extends Controller
{
    ...
    public function apply(Request $request)
    {
        try {
            ...
            $basicSetting = ApplySetting::where('store_id', $storeId)
                ->first();
            $requiredParams = [];
            ...
            $input = $this->validate($request, $requiredParams);
    ...
    public function info(Request $request)
    {
        try {
            ...
            $data = Member::with('user')
                ->with([
                    'level' => function(HasOne $query) use ($storeId)
                    {
            ...
            if(isset($data)) {
                ...
                $memberInfo['join_time'] = $data->join_time >=1 ? date('Y-m-d H:i:s',$data->join_time) : "-";
                // 分销订单金额总计
    ...
    public function subordinate(Request $request)
    {
        try {
            ...
            $validFields = array('1'=>'father_id', '2'=>'grand_father_id', '3'=>'great_grand_father_id');
            ...
            $data['list'] = Member::with('user')
                ...
                ->map(function( Member $item ) {
                    $userId = $item->user->id;
                    $order = Order::select(\DB::raw('count(*) as order_count, sum(total_price) as totalPrice'))
                    ...
                    if($order){
                        ...
                        $item->total_price = number_format($order->totalPrice, 2);
                    }
                    // 分销订单金额总计
